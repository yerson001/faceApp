...
class Face_Register:
    ...
    def GUI_info(self):
        ...
        tk.Label(self.frame_right_info, text="Caras Registradas: ").grid(row=2, column=0, sticky=tk.W, padx=5, pady=2)
        self.label_cnt_face_in_database.grid(row=2, column=1, sticky=tk.W, padx=5, pady=2)

        tk.Label(self.frame_right_info,
                 text="Faces actual: ").grid(row=3, column=0, columnspan=2, sticky=tk.W, padx=5, pady=2)
        ...
        tk.Label(self.frame_right_info,
                 ...
                 text="Step 1: Eliminar fotos").grid(row=5, column=0, columnspan=2, sticky=tk.W, padx=5, pady=20)
        ...
        tk.Label(self.frame_right_info,
                 font=self.font_step_title,
                 text="Step 2: Ingrese Nombre").grid(row=7, column=0, columnspan=2, sticky=tk.W, padx=5, pady=20)

        tk.Label(self.frame_right_info, text="Nombre: ").grid(row=8, column=0, sticky=tk.W, padx=5, pady=0)
        ...
        tk.Label(self.frame_right_info,
                 font=self.font_step_title,
                 text="Step 3: Guadar Imagen ").grid(row=9, column=0, columnspan=2, sticky=tk.W, padx=5, pady=20)
        ...
        if self.face_folder_created_flag:
            if self.current_frame_faces_cnt == 1:
                if not self.out_of_range_flag:
                    ...
                    self.log_all["text"] = "NO fuerad de rango!"
            else:
                self.log_all["text"] = "No face!"
        else:
            self.log_all["text"] = "paso dos!"

    def get_frame(self):
        try:
            if self.cap.isOpened():
                ret, frame = self.cap.read()
                frame = cv2.resize(frame, (640,480))#normalizar
